...
export default async function caxa({
  ...
  includeNode = true,
  stub = url.fileURLToPath(new URL(`../stubs/stub--${process.platform}--${process.arch}`, import.meta.url)),
  ...
  if (dedupe) await execa("npm", ["dedupe", "--production"], { cwd: buildDirectory })
  ...
  if (output.endsWith(".app")) {
    if (process.platform !== "darwin") throw new Error("macOS Application Bundles (.app) are supported in macOS only.")
    ...
    await fs.writeFile(
      ...
      bash`
        #!/usr/bin/env sh
        ${command.map((part) => `"${part.replace(/\{\{\s*caxa\s*\}\}/g, `$(dirname "$0")/application`)}"`).join(" ")}
      ` + "\n",
      { mode: 0o755 }
    )
    await fs.rename(buildDirectory, path.join(output, "Contents/Resources/application"))
  } else if (output.endsWith(".sh")) {
    if (process.platform === "win32") throw new Error("The Shell Stub (.sh) isn’t supported in Windows.")
    let stub =
      bash`
        #!/usr/bin/env sh
        ...
        do
          ...
          then
            ...
            ${uncompressionMessage === undefined ? bash`` : bash`echo "${uncompressionMessage}" >&2`}
            mkdir -p "$CAXA_LOCK"
  ...
  if (removeBuildDirectory) await fs.rm(buildDirectory, { recursive: true, force: true })
...
if (process.env.TEST === "caxa") {
  ...
  if (process.platform === "darwin")
    await (async () => {
      ...
      assert.equal(
        (await execa(path.join(output, "/Contents/Resources/Echo Command Line Parameters"), { all: true })).all,
        dedent`
          [
            "some",
            "embedded arguments"
